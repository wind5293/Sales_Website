...
// Tính form mặc định 1 lần từ danh sách địa chỉ ban đầu (server truyền xuống)
const getDefaultForm = (addresses) => {
    const def = addresses.find(a => a.is_default) || addresses[0];
    if (!def) return { name: '', phone: '', address: '' };
    return {
        name: def.name || '',
        phone: def.phone || '',
        address: [def.street, def.district, def.city].filter(Boolean).join(', '),
    };
};

const CheckoutClient = ({ initialAddresses }) => {
    const router = useRouter();
    const { items, totalItems, totalPrice, fetchCart } = useCart();

    const [addresses] = useState(initialAddresses);
    const [form, setForm] = useState(() => getDefaultForm(initialAddresses));
    ...
    const openVoucherModal = async () => {
        ...
        try {
            const res = await fetch(`/api/coupons/available?order_total=${totalPrice}`);
            const data = await res.json();
            setAvailableVouchers(data.coupons ?? []);
        } catch {
    ...
    const handleApplyVoucher = async () => {
        ...
        try {
            const res = await fetch('/api/coupons/validate', {
                ...
                body: JSON.stringify({ voucherCode: code, orderTotal: totalPrice }),
            });
    ...
    const handleApplyVoucherDirect = (v) => {
        const discount = v.discountPercent
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (!res.ok) {
                setErrors({ submit: data.detail || 'Đặt hàng thất bại. Vui lòng thử lại.' });
                return;
            }

            setOrderSuccess({ orderId: data.orderId, totalPrice: data.totalPrice });
            await fetchCart();
        } catch {
            ...
            <div className="bg-[#f5f5f5] min-h-screen flex items-center justify-center px-4 py-12">
                <div className="bg-white rounded-sm shadow max-w-lg w-full p-10 text-center">
                    ...
                    <p className="text-slate-500 text-sm mb-8">
                        ...
                        <span className="text-[#f59e0b] font-semibold"> electro.</span>
                    </p>
                    ...
                    <Link href="/" className="block w-full bg-[#f59e0b] hover:bg-[#d97706] text-white font-bold py-3 rounded-sm text-sm text-center transition-colors">
                        Tiếp tục mua sắm
                    </Link>
                </div>
            </div>
        ...
        return (
            <div className="bg-[#f5f5f5] min-h-screen flex items-center justify-center px-4">
                <div className="text-center bg-white p-12 rounded-sm shadow">
                    ...
                    <Link href="/" className="bg-[#f59e0b] hover:bg-[#d97706] text-white font-semibold text-sm px-8 py-3 rounded-sm transition-colors inline-block">
                        Mua ngay
                    </Link>
                </div>
            </div>
    ...
    return (
        <div className="bg-[#f5f5f5] min-h-screen">
            ...
            <div className="max-w-5xl mx-auto px-4 pt-5 pb-2 flex items-center gap-2 text-xs">
                <Link href="/" className="flex items-center gap-1 text-slate-400 hover:text-[#f59e0b] transition-colors">
                ...
                <div className="bg-white">
                    ...
                    <div className="px-6 py-4 flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <button onClick={openVoucherModal} className="text-sm text-[#f59e0b] hover:underline font-medium">
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4">
                    <div className="bg-white shadow-xl w-full max-w-md">
                        ...
                        <div className="p-5 space-y-4">
                            ...
                            <div className="space-y-2">
                                <p className="text-xs text-slate-400 font-medium tracking-wide">Voucher khả dụng</p>
                                ...
                                {loadingVouchers ? (
                                    ...
                                    <p className="text-xs text-slate-400 text-center py-4">Không có voucher khả dụng</p>
                                ) : (
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center px-4">
                    <div className="bg-white shadow-xl w-full max-w-lg flex flex-col max-h-[80vh]">
                        ...
                        <div className="overflow-y-auto flex-1 p-5 space-y-3">
                            ...
                            <button
                                onClick={() => { setShowAddressModal(false); router.push('/profile/addresses'); }}
