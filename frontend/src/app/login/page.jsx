...
export default function LoginForm() {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        setMessage(''); setError('');
        if (!email.trim() || !password.trim()) { setError('Vui lòng điền email và mật khẩu!'); return; }
        ...
        try {
            ...
            if (!res.ok) { setError(data.detail || 'Email hoặc mật khẩu không đúng!'); return; }

            setMessage(data.message.startsWith('🛡️') ? data.message : `🎉 ${data.message}`);
            await fetchCart();

            setTimeout(() => {
                router.push('/');
                router.refresh();
            }, data.message.startsWith('🛡️') ? 1500 : 2000);
        } catch {
                ...
                <div className="w-full max-w-[450px] bg-white p-8 sm:p-10 rounded-1xl border border-gray-100">
                    ...
                    <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        <div className="flex items-center justify-between text-xs pt-1">
                            ...
                            <a href="#" className="font-medium text-gray-700 hover:underline">Quên mật khẩu?</a>
                        </div>

                        <button
                            type="submit" disabled={loading}
                            className="w-full py-2 bg-amber-400 hover:bg-amber-500 text-gray-900 font-semibold transition shadow-sm text-sm mt-2 disabled:opacity-60"
                        >
