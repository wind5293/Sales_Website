...
const AddressBook = () => {
    ...
    const handleSave = async (e) => {
        ...
        try {
            const token = getToken();
            if (editingId) {
                await axios.patch(`/api/users/addresses/${editingId}`, form, {
                    headers: { Authorization: `Bearer ${token}` }
                });
                ...
                showAlert('success', 'Thêm địa chỉ mới thành công!');
            }
            closeModal();
            fetchAddresses();
        } catch {
    ...
    const handleSetDefault = async (addr) => {
        ...
        try {
            const token = getToken();
            await axios.patch(`/api/users/addresses/${addr.address_id}`, { is_default: true }, {
                headers: { Authorization: `Bearer ${token}` }
            });
    ...
    return (
        <div className="bg-slate-50 min-h-screen font-sans py-10">
            <div className="max-w-4xl mx-auto px-4">
                ...
                {/* Alert */}
                {alert.message && (
                    <div className={`mb-6 p-4 flex items-center gap-3 text-sm font-medium ${alert.type === 'success'
                ...
                <div className="bg-white rounded-sm border border-slate-100 overflow-hidden">
                    ...
                    <div className="bg-gradient-to-r from-slate-900 to-slate-800 px-8 py-6 flex items-center justify-between">
                        <div className="flex items-center gap-4">
                            ...
                            <div>
                                <h1 className="text-xl font-bold text-white tracking-wide">Địa chỉ của Tôi</h1>
                            </div>
                        </div>
                        <button
                                ...
                                {addresses.map((addr) => (
                                    ...
                                    >
                                        ...
                                        <div className="flex items-center gap-3 mb-1">
                                            {addr.name && (
                                                <span className="font-bold text-slate-600 text-sm">{addr.name}</span>
                                            )}
                                            {addr.phone && (
                                                <>
                                        ...
                                        {/* Badge mặc định */}
                                        {addr.is_default && (
                                            <span className="inline-block mt-2 px-2 py-0.5 border border-amber-500 text-amber-500 text-[11px] font-medium rounded-sm">
                                                Mặc định
                                            </span>
                                        )}
                        ...
                        <form onSubmit={handleSave} className="p-6 space-y-4">
                            ...
                            <div className="pt-2 flex items-center justify-end gap-3">
                                <button
                                    type="button"
                                    onClick={closeModal}
                                    className="px-5 py-2 text-sm font-semibold text-slate-600 hover:bg-slate-100 rounded-sm transition-colors"
                                >
                                    Hủy
                                </button>
                                <button
                                ...
                                >
                                    {saving ? (
                                        ...
                                        <><i className="far fa-save"></i>{editingId ? 'Lưu thay đổi' : 'Thêm địa chỉ'}</>
                                    )}
                                </button>
                            </div>
                        </form>
